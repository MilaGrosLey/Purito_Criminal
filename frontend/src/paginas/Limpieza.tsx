  ...
  const cargarDatos = async () => {
    try {
      const [respCol, respPrev, respCorr, respDist, respEst] = await Promise.all([
        ...
        axios.get(`http://localhost:5000/api/datasets/${id}/vista-previa?pagina=1`),
      ...
      setPaginaActual(respPrev.data.pagina_actual)
      setTotalPaginas(respPrev.data.total_paginas)
      setTotalFilas(respPrev.data.total_filas)
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center">
                  <svg className="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3a 11-6 0 11-6z" />
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                  </svg>
                </div>
